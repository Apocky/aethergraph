...
At broad zoom levels, nearby notes pool into soft semantic light regions with one crisp representative label. Labels are selected from owner/inferred document subjects, topics, tags, term-level attributed provider subjects, and controlled facets using visible-corpus specificity; legacy `label_source:none` extraction words, node-level bank participation, and structural folder contexts cannot become topical headings. Labels are globally deduplicated, measured for collision, and limited to an adaptive 18–48 per viewport. Zooming in resolves pools back toward individual document titles.
